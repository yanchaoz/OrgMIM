...
class SparK(nn.Module):
    ...
    @torch.no_grad()
    def generate_mask_mam(self, att_map: torch.Tensor, step: int = 0, total_step: int = 400000, patch_size: int = 16, alpha_t: float = 0.75) -> torch.Tensor:
        h, w, d= self.fmap_h, self.fmap_w, self.fmap_d
        att_map = F.avg_pool3d(att_map, kernel_size=patch_size, stride=patch_size)
        att_map = 1 - self.patchify_att(att_map).squeeze()
        # print(att_map.shape, att_map.dim())
        # if att_map.dim() == 1:
        ...
        if int((L - self.len_keep) * keep_ratio) <= 0:
            ...
            for i in range(B):
                ...
                ids_shuffle2 = torch.zeros_like(ids_shuffle_att, device=att_map.device).int()
                ids_shuffle2[i, -len_loss-easy_len:-len_loss] = ids_shuffle_att[i, -len_loss-easy_len:-len_loss]
                temp = torch.arange(L, device=att_map.device)
                deleted = np.delete(temp.cpu().numpy(), ids_shuffle2[i, -len_loss-easy_len:-len_loss].cpu().numpy())
                np.random.shuffle(deleted)
                ids_shuffle2[i, :(L - easy_len)] = torch.LongTensor(deleted).to(att_map.device)
            
        ids_restore = torch.argsort(ids_shuffle, dim=1)
        mask = torch.zeros([B, L], device=att_map.device, dtype=torch.bool) 
        mask[:, :self.len_keep] = 1 
    ...
    def generate_mask_alm(self, loss_pred: torch.Tensor, step: int = 0, total_step: int = 400000, alpha_t: float = 0.75) -> torch.Tensor:
        ...
        ids_restore = torch.argsort(ids_shuffle, dim=1)
        mask = torch.zeros([B, L], device=loss_pred.device, dtype=torch.bool) 
        mask[:, :self.len_keep] = 1 
        mask = torch.gather(mask, dim=1, index=ids_restore)
