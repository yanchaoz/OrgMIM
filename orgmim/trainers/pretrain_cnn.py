...
import torch
from ..utils.visual2d_cnn import visual_2d
from timm.utils import ModelEma
from ..mim_cnn.encoder3D import SparseEncoder
from ..mim_cnn.stunet_head import STUNet_base, STUNet_small, STUNet_large
from ..mim_cnn.spark_orgmim import SparK
from ..mim_cnn.decoder3D import LightDecoder
import cv2
...
def loop(cfg, train_provider, iters, writer):
    ...
    if cfg.MODEL.scale == "base":
        head = STUNet_base(1, 1)
    elif cfg.MODEL.scale == "small":
        head = STUNet_small(1, 1)
    elif cfg.MODEL.scale == "large":
        head = STUNet_large(1, 1)
    else:
        raise ValueError(cfg.MODEL.scale)
